...
def _yield_text_items(x: Any) -> Iterable[str]:
    ...
    # New + mixed schemas: lists of items
    if isinstance(x, list):
        for v in x:
            yield from _yield_text_items(v)
        return

    # Dict handling: prefer 'text' / 'content' fields
    if isinstance(x, dict):
        # 1) direct 'text'
        if 'text' in x and isinstance(x['text'], str):
            yield x['text']

        # 2) 'content' can be str OR nested dict with 'text'
        if 'content' in x:
            c = x['content']
            if isinstance(c, str):
                yield c
                ...
                if 'text' in c and isinstance(c['text'], str):
                    ...
                    # If content holds more nesting, scan it for text
                    # (still ignore numbers like scores)
                    for v in c.values():
                        if isinstance(v, str):
                            yield v
        ...
        # 3) As a fallback for unexpected nesting, scan other fields that are strings
        #    (but do NOT add numbers like scores)
        for k, v in x.items():
    ...
    # Old schema: direct strings
    if isinstance(x, str):
        yield x
        return

    # Everything else (numbers, bools, None) -> ignore
    return

def store_text_set(store_data):
    out = set()
...
def evaluate_all(
    indices = range(70),
    path_google: str = "output_all_gpt",
    path_gpt4o: str = "output_async_react_gpt-4o-mini",
    hist_search_path: str = "/home/adsl-1-3/Documents/CNN_web_crawler-master/historical_search_results.json",
):
    ...
    for i in indices:
        ...
        # Unique normalized sets for predictions (handles both schemas)
        rs_set      = unique_norm_set(react_search)
        rstore_set  = store_text_set(react_store)
        nrs_set     = unique_norm_set(new_search)
        nrstore_set = store_text_set(new_store)
        ...
        # Optional: store gold size if you later want weighted averages
    ...
    # Save outputs
    write_json("per_id_scores.json", results)
    write_json("average_scores.json", avg_scores)
    write_json("f1_scores.json", f1_per_id)
